...
<body style="font-family:Arial,sans-serif;background:#f4f4f4;padding:40px 0;">
  <div style="max-width:480px;margin:auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1);">
    <div style="background:#059669;padding:24px 32px;">
      <h1 style="color:#fff;margin:0;font-size:20px;">eVote — Scrutin ouvert</h1>
    </div>
    <div style="padding:32px;">
      <p style="margin:0 0 16px;">Bonjour <strong>${recipientName}</strong>,</p>
      <p style="margin:0 0 24px;">Le scrutin <strong>«&nbsp;${electionTitle}&nbsp;»</strong> vient d'ouvrir. Vous pouvez maintenant voter.</p>
      <a href="${loginUrl}" style="display:inline-block;background:#059669;color:#fff;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:15px;">Accéder au vote</a>
    ...
    await this.sendMail({
      to,
      subject: `Scrutin ouvert : ${electionTitle}`,
      html,
      text: `Bonjour ${recipientName},\n\nLe scrutin « ${electionTitle} » est maintenant ouvert. Connectez-vous pour voter :\n${loginUrl}`,
    });
  }
}
