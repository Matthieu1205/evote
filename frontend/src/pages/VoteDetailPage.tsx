        ...
        return (
          <div key={step.key} className="flex flex-1 items-center">
            <div className="flex flex-col items-center">
              ...
              <span className={`mt-1 hidden text-[10px] font-semibold sm:block ${active ? 'text-brand-700' : 'text-ink-400'}`}>
                {step.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`mx-1 h-0.5 flex-1 ${i < idx ? 'bg-brand-500' : 'bg-ink-200'}`} />
            )}
          </div>
        );
      })}
...
export default function VoteDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [electionData, voteCheck] = await Promise.all([
        ...
        api.get<{ hasVoted: boolean }>(`/votes/check/${electionId}`).catch(() => ({ hasVoted: false })),
      ]);
  ...
  if (loading) return <AppShell><div className="h-40 animate-pulse rounded-2xl bg-ink-100" /></AppShell>;
  if (!election) return <AppShell><p className="text-ink-400">Scrutin introuvable.</p></AppShell>;

  const showResults = ['DEPOUILLE', 'PUBLIE'].includes(election.status);
  const validCandidacies = (p: Position) => p.candidacies.filter((c) => c.status === 'VALIDEE');
  ...
  return (
    <AppShell>
      {/* Breadcrumb */}
      <Link to="/vote" className="inline-flex items-center gap-1 text-sm font-medium text-ink-400 transition hover:text-brand-600 mb-4">
      ...
      {showResults ? (
        <div className="flex flex-col items-center rounded-2xl bg-white py-16 text-center shadow-sm ring-1 ring-ink-100">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-teal-100">
            <svg viewBox="0 0 24 24" className="h-8 w-8 text-teal-600" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              <line x1="18" y1="20" x2="18" y2="10" />
              <line x1="12" y1="20" x2="12" y2="4" />
              <line x1="6" y1="20" x2="6" y2="14" />
            </svg>
          </div>
          <h2 className="mt-4 text-lg font-bold text-ink-900">Résultats publiés</h2>
          <p className="mt-1 text-sm text-ink-500">Consultez les résultats officiels du scrutin.</p>
          <Link to={`/vote/${electionId}/results`} className="mt-6 rounded-xl bg-brand-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
            Voir les résultats
          </Link>
        </div>
      ...
      /* VOTE DÉJÀ ENREGISTRÉ */
        ...
        <div className="flex flex-col items-center rounded-2xl bg-white py-16 text-center shadow-sm ring-1 ring-ink-100">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-100">
            <svg viewBox="0 0 24 24" className="h-8 w-8 text-brand-600" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
              <path d="M5 12l5 5L20 7" />
            </svg>
          </div>
          <h2 className="mt-4 text-xl font-bold text-ink-900">Vote enregistré !</h2>
          <p className="mt-2 max-w-sm text-sm text-ink-500">
            Votre bulletin a été chiffré et déposé de manière anonyme. Merci de votre participation.
          </p>
          <Link to="/vote" className="mt-6 rounded-xl bg-brand-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
            Retour aux scrutins
          </Link>
        </div>
      ...
      /* PAS ENCORE OUVERT */
        ...
        <div className="space-y-5">
          <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <svg viewBox="0 0 24 24" className="mt-0.5 h-5 w-5 shrink-0 text-amber-500" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              <circle cx="12" cy="12" r="10" />
              <line x1="12" y1="8" x2="12" y2="12" />
              <line x1="12" y1="16" x2="12.01" y2="16" />
            </svg>
            <div>
              <p className="font-semibold text-amber-800">Le vote n'est pas encore ouvert</p>
              <p className="mt-0.5 text-sm text-amber-700">
                Ouverture prévue le{' '}
                <strong>{new Date(election.startAt).toLocaleDateString('fr-FR', { dateStyle: 'long' })}</strong>{' '}
                à {new Date(election.startAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}.
              </p>
            </div>
          </div>
          {election.positions.map((p) => {
            ...
            return (
              <div key={p.id} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-ink-100">
                <div className="border-b border-ink-100 px-5 py-4">
                  <h3 className="font-semibold text-ink-900">{p.title}</h3>
                  <p className="mt-0.5 text-xs text-ink-400">{p.seats} siège{p.seats > 1 ? 's' : ''}</p>
                </div>
                <div className="divide-y divide-ink-50">
                  {cands.length === 0 ? (
                    ...
                    <div key={c.id} className="flex items-center gap-3 px-5 py-3.5">
                      {c.photoUrl ? (
                        <img src={c.photoUrl} alt="" className="h-10 w-10 shrink-0 rounded-full border border-ink-100 object-cover" />
                      ) : (
                      ...
                      <div>
                        <p className="font-semibold text-ink-900">{c.user.firstName} {c.user.lastName}</p>
                        <p className="text-xs text-ink-400">{c.profession ? `${c.profession} · ${c.user.ordreNumber}` : c.user.ordreNumber}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              return (
                <div key={p.id} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-ink-100">
                  <div className="flex items-center justify-between border-b border-ink-100 px-5 py-4">
                    ...
                    <span className={`text-xs font-semibold ${selected.length > 0 ? 'text-brand-600' : 'text-ink-400'}`}>
                      {p.seats > 1 ? `${selected.length}/${p.seats}` : selected.length === 1 ? '✓' : '—'}
                    </span>
                  </div>
                  <div className="divide-y divide-ink-50">
                    {cands.length === 0 && (
                    ...
                    {cands.map((c) => {
                      ...
                      return (
                        ...
                        >
                          {c.photoUrl ? (
                            <img src={c.photoUrl} alt="" className="h-11 w-11 shrink-0 rounded-xl border border-ink-100 object-cover" />
                          ) : (
                            <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-sm font-bold ${isSel ? 'bg-brand-200 text-brand-800' : 'bg-brand-100 text-brand-600'}`}>
                              {c.user.firstName[0] ?? ''}{c.user.lastName[0] ?? ''}
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            <p className={`font-semibold ${isSel ? 'text-brand-900' : 'text-ink-900'}`}>
                              {c.user.firstName} {c.user.lastName}
                            </p>
                            <p className="truncate text-xs text-ink-400">
                              {c.profession ? `${c.profession} · ${c.user.ordreNumber}` : c.user.ordreNumber}
                            </p>
                          </div>
                          {/* Checkbox / radio */}
                          <div className={`flex h-5 w-5 shrink-0 items-center justify-center ${p.seats > 1 ? 'rounded-md' : 'rounded-full'} border-2 transition ${
                            isSel ? 'border-brand-600 bg-brand-600 text-white' : 'border-ink-200'
                          }`}>
                            {isSel && (
                              <svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round">
                                <path d="M5 12l5 5L20 7" />
                              </svg>
                            )}
                          </div>
            ...
            <div className="sticky top-24 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-ink-100">
              {phase === 'ballot' && (
                <div className="p-5">
                  ...
                  <p className="mt-1 text-xs text-ink-400">Sélectionnez vos candidats pour chaque poste.</p>
                  <button
                    type="button"
              ...
              {phase === 'review' && (
                <div className="p-5">
                  ...
                  <p className="mt-1 text-xs text-ink-400">Vérifiez vos choix avant de confirmer.</p>

                  {blankPositions.length > 0 && (
                    <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                      <strong>Vote blanc :</strong> aucun choix pour {blankPositions.map((p) => `"${p.title}"`).join(', ')}.
                    </div>
                  )}
                    ...
                    {election.positions.map((p) => {
                      ...
                      return (
                        <div key={p.id} className="rounded-xl bg-ink-50 px-3 py-2.5">
                          <p className="text-xs font-semibold text-ink-600">{p.title}</p>
                          {sel.length === 0 ? (
                            <p className="mt-0.5 text-xs italic text-ink-400">Vote blanc</p>
                          ) : (
                            <ul className="mt-1 space-y-0.5">
                              {sel.map((c) => (
                                <li key={c.id} className="flex items-center gap-1.5 text-xs font-medium text-brand-700">
                  ...
                  {error && <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs text-red-700">{error}</div>}
                  ...
                  <button
                    ...
                    onClick={() => { setPhase('ballot'); setError(null); }}
                  >
                    ← Modifier mes choix
                  </button>
                </div>
              ...
              {phase === 'otp' && (
                <div className="p-5">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-100">
                    <svg viewBox="0 0 24 24" className="h-5 w-5 text-brand-600" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                  ...
                  <h3 className="mt-3 font-bold text-ink-900">Code de confirmation</h3>
                  <p className="mt-1 text-xs text-ink-400">Un code OTP à 6 chiffres a été envoyé à votre adresse email.</p>

                  {devCode && (
                    <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700">
                      Code de démonstration : <b className="font-mono">{devCode}</b>
                    </div>
                  )}
                  {error && <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs text-red-700">{error}</div>}

                  <div className="mt-4">
                    <label className="mb-1.5 block text-xs font-semibold text-ink-600">Votre code OTP</label>
                    <input
