...
export function BackofficeShell({ children }: { children: ReactNode }) {
      ...
      <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
        ...
        <div>
          ...
          <p className="truncate text-sm font-bold text-white">
            {user?.organization.name ?? "Administration"}
          </p>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
        {NAV.map((item) => (
          ...
          >
            <span
              className={
                isActive(item.href) ? "text-emerald-300" : "text-white/40"
              }
            >
              {item.icon}
            </span>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      {/* Drawer mobile */}
      <aside
        className={`fixed left-0 top-0 z-50 h-full w-80 transition-transform duration-300 lg:hidden ${
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          ...
          <img
            ...
            style={{
              filter:
                "brightness(0) saturate(100%) invert(39%) sepia(88%) saturate(500%) hue-rotate(124deg)",
            }}
          />
          <div className="w-9" />
        </header>
